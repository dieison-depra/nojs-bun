...
describe("else-if directive", () => {
    ...
    `;
		...
		expect(
			document.getElementById("w").querySelector(".warn-msg"),
		).not.toBeNull();
	});
});
...
describe("else directive (standalone)", () => {
    ...
    `;
		...
		expect(
			document.getElementById("elseEl").querySelector("p").textContent,
		).toBe("Fallback");
	});
    ...
    `;
		...
		expect(document.getElementById("ifEl").querySelector("p").textContent).toBe(
			"Visible",
		);
    ...
    `;
		...
		expect(document.getElementById("warn").innerHTML).toBe("");
		expect(
			document.getElementById("fallback").querySelector("p").textContent,
		).toBe("All good");
	});
    ...
    `;
		...
		expect(
			document.getElementById("elseEl").querySelector(".fb"),
		).not.toBeNull();
	});
});
...
describe("state persist directive", () => {
	...
	test("restores state from localStorage", () => {
		localStorage.setItem("nojs_state_test2", JSON.stringify({ name: "saved" }));
...
describe("else-if directive — uncovered branches", () => {
	...
	test("else-if with then template for the truthy branch", () => {
		...
		expect(elseIfEl.querySelector(".elseif-rendered").textContent).toBe(
			"Rendered via then",
		);
	});
