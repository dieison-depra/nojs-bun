...
describe("Use Directive (Templates)", () => {
	...
	test("handles slots", () => {
		...
		tpl.id = "slot-tpl";
		tpl.innerHTML =
			'<div class="header"><slot name="header"></slot></div><div class="body"><slot></slot></div>';
		document.body.appendChild(tpl);
...
describe("HTTP GET with error template", () => {
	...
	test("shows error template on failed request", async () => {
		global.fetch.mockResolvedValue(
			mockErrorResponse(500, { message: "HTTP 500" }),
		);
...
describe("use directive — slots", () => {
	...
	test("use handles named slots", () => {
		...
		tpl.id = "layout";
		tpl.innerHTML =
			'<header><slot name="header"></slot></header><main><slot></slot></main><footer><slot name="footer"></slot></footer>';
		document.body.appendChild(tpl);
...
describe("Form validation — dirty and touched tracking", () => {
	...
	test("submitting becomes true on submit", async () => {
		...
		const form = document.createElement("form");
		form.setAttribute("validate", "");
		form.innerHTML = '<input name="x" /><button type="submit">Submit</button>';
		parent.appendChild(form);
		document.body.appendChild(parent);
...
describe("custom validator error", () => {
	...
	test("custom validator returns error message when validation fails", async () => {
		...
		form.setAttribute("validate", "");
		form.innerHTML =
			'<input name="username" validate="noSpaces" value="has space" />';
		parent.appendChild(form);
		document.body.appendChild(parent);
...
describe("use directive — slot without matching content", () => {
	...
	test("slot element remains when no matching content is provided", () => {
		...
		tpl.innerHTML =
			'<div class="main"><slot></slot></div><aside><slot name="sidebar"></slot></aside>';
		document.body.appendChild(tpl);
...
describe("Validation — custom validator via _validators", () => {
	...
	test("custom validator is called and returns error", async () => {
		_validators.myRule = jest.fn((val) =>
			val !== "valid" ? "Must be valid" : null,
		);
...
describe("Validation Revamp — Per-rule error messages", () => {
	...
	test("error-{rule} provides custom message", async () => {
		...
		form.setAttribute("validate", "");
		form.innerHTML =
			'<input name="email" validate="required|email" error-required="Email is required" error-email="Invalid email format" />';
		parent.appendChild(form);
		document.body.appendChild(parent);
		processTree(parent);
		await new Promise((r) => setTimeout(r, 50));
	...
	test("generic error attribute as fallback", async () => {
		...
		form.setAttribute("validate", "");
		form.innerHTML =
			'<input name="val" validate="required" error="This field has an error" />';
		parent.appendChild(form);
		document.body.appendChild(parent);
		processTree(parent);
		await new Promise((r) => setTimeout(r, 50));
	...
	test("error-{rule} takes priority over generic error", async () => {
		...
		form.setAttribute("validate", "");
		form.innerHTML =
			'<input name="val" validate="required" error="Generic" error-required="Specific" />';
		parent.appendChild(form);
		document.body.appendChild(parent);
		processTree(parent);
		await new Promise((r) => setTimeout(r, 50));
...
describe("Validation Revamp — Auto-disable submit buttons", () => {
	...
	test("submit button disabled when form invalid", async () => {
		...
		form.setAttribute("validate", "");
		form.innerHTML =
			'<input name="x" validate="required" /><button type="submit">Go</button>';
		parent.appendChild(form);
		document.body.appendChild(parent);
		processTree(parent);
		await new Promise((r) => setTimeout(r, 50));
	...
	test("submit button enabled when form valid", async () => {
		...
		form.setAttribute("validate", "");
		form.innerHTML =
			'<input name="x" validate="required" value="ok" /><button type="submit">Go</button>';
		parent.appendChild(form);
		document.body.appendChild(parent);
		processTree(parent);
		await new Promise((r) => setTimeout(r, 50));
	...
	test('button type="button" is NOT auto-disabled', async () => {
		...
		form.setAttribute("validate", "");
		form.innerHTML =
			'<input name="x" validate="required" /><button type="button">Cancel</button>';
		parent.appendChild(form);
		document.body.appendChild(parent);
		processTree(parent);
		await new Promise((r) => setTimeout(r, 50));
	...
	test("input[type=submit] is auto-disabled", async () => {
		...
		form.setAttribute("validate", "");
		form.innerHTML =
			'<input name="x" validate="required" /><input type="submit" value="Send" />';
		parent.appendChild(form);
		document.body.appendChild(parent);
		processTree(parent);
		await new Promise((r) => setTimeout(r, 50));
	...
	test("button without type is auto-disabled (defaults to submit)", async () => {
		...
		form.setAttribute("validate", "");
		form.innerHTML =
			'<input name="x" validate="required" /><button>Submit</button>';
		parent.appendChild(form);
		document.body.appendChild(parent);
		processTree(parent);
		await new Promise((r) => setTimeout(r, 50));
...
describe("Validation Revamp — error-class", () => {
	...
	test("per-field error-class overrides form-level", async () => {
		...
		form.innerHTML =
			'<input name="x" validate="required" error-class="field-error" />';
		parent.appendChild(form);
		document.body.appendChild(parent);
		processTree(parent);
		await new Promise((r) => setTimeout(r, 50));
...
describe("Validation Revamp — validate-on", () => {
	...
	test("per-field validate-on overrides form-level", async () => {
		...
		form.setAttribute("validate-on", "blur");
		form.innerHTML =
			'<input name="x" validate="required" validate-on="input" value="" />';
		parent.appendChild(form);
		document.body.appendChild(parent);
		processTree(parent);
		await new Promise((r) => setTimeout(r, 50));
...
describe("Validation Revamp — validate-if", () => {
	...
	test("skips validation when validate-if is false", async () => {
		...
		form.setAttribute("validate", "");
		form.innerHTML =
			'<input name="company" validate="required" validate-if="show" />';
		parent.appendChild(form);
		document.body.appendChild(parent);
		processTree(parent);
		await new Promise((r) => setTimeout(r, 50));
...
describe("Validation Revamp — $form.fields", () => {
	...
	test('as="" attribute exposes field context on parent', async () => {
		...
		form.setAttribute("validate", "");
		form.innerHTML =
			'<input name="email" validate="required" as="emailField" />';
		parent.appendChild(form);
		document.body.appendChild(parent);
		processTree(parent);
		await new Promise((r) => setTimeout(r, 50));
...
describe("Validation Revamp — Error template references", () => {
	...
	test('error-{rule}="#tpl" renders template in-place', async () => {
		...
		form.innerHTML =
			'<input name="x" validate="required" error-required="#req-tpl" />';
	...
	test("clears template when field becomes valid", async () => {
		...
		form.innerHTML =
			'<input name="x" validate="required" error-required="#req-tpl2" />';
		form.appendChild(tpl);
		parent.appendChild(form);
		document.body.appendChild(parent);
		processTree(parent);
