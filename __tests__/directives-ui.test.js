...
describe("Event Directive (on:*)", () => {
	...
	test("provides $event and $el", () => {
		...
		btn.setAttribute(
			"on:click",
			"eventType = $event.type; tagName = $el.tagName",
		);
		parent.appendChild(btn);
		document.body.appendChild(parent);
		processTree(parent);
...
describe("Trigger Directive", () => {
	...
	test("dispatches custom event with data", () => {
		...
		let detail = null;
		parent.addEventListener("select-item", (e) => {
			detail = e.detail;
		});
		btn.click();
...
describe("on:click with throttle", () => {
	...
	test("throttles handler execution", async () => {
		const parent = document.createElement("div");
		parent.setAttribute("state", "{ count: 0 }");
		const btn = document.createElement("button");
		// Use a smaller throttle for faster testing
		btn.setAttribute("on:click.throttle.50", "count++");
		parent.appendChild(btn);
		document.body.appendChild(parent);
		...
		btn.click(); // 1st click: immediate execution
		expect(ctx.count).toBe(1);

		btn.click(); // 2nd click: throttled (ignored)
		expect(ctx.count).toBe(1);

		await new Promise((r) => setTimeout(r, 60)); // Wait for throttle window

		btn.click(); // 3rd click: execution allowed
...
describe("each directive — uncovered branches", () => {
	...
	test('each with "animate" attribute fallback (not animate-enter)', () => {
		...
		expect(wrappers[0].firstElementChild.classList.contains("slideIn")).toBe(
			true,
		);
		expect(wrappers[1].firstElementChild.classList.contains("slideIn")).toBe(
			true,
		);
	});
...
describe("keyup event modifiers", () => {
	...
	test("keyup.meta requires meta key", () => {
		...
		input.dispatchEvent(
			new KeyboardEvent("keyup", { key: "a", metaKey: true, bubbles: true }),
		);
		expect(findContext(parent).meta).toBe(true);
	});
});
