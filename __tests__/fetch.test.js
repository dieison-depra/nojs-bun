import { _cacheGet, _cacheSet, _doFetch, resolveUrl } from "../src/fetch.js";
import { _cache, _config, _interceptors } from "../src/globals.js";
...
describe("URL Resolution", () => {
	...
	test("returns absolute URLs unchanged", () => {
		expect(resolveUrl("https://api.example.com/users", null)).toBe(
			"https://api.example.com/users",
		);
		expect(resolveUrl("http://localhost:3000/data", null)).toBe(
			"http://localhost:3000/data",
		);
		expect(resolveUrl("//cdn.example.com/script.js", null)).toBe(
...
describe("Fetch (_doFetch)", () => {
	...
	test("applies request interceptors", async () => {
		...
		expect(global.fetch.mock.calls[0][1].headers["X-Custom"]).toBe(
			"intercepted",
		);
	});
	...
	test("throws AbortError on abort", async () => {
		...
		await expect(
			_doFetch("/api/test", "GET", null, {}, null, controller.signal),
