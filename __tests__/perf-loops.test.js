...
describe("Phase 4: Template Cloning Engine", () => {
	...
	it("should clone from template specified by data-nojs-template", async () => {
		...
		const li1 =
			list.children[0].querySelector("li") ||
			list.children[0].firstElementChild;
		expect(li1.textContent).toBe("A");

		const li2 =
			list.children[1].querySelector("li") ||
			list.children[1].firstElementChild;
	...
	it("should work with foreach and data-nojs-template", async () => {
		...
		const span1 =
			div.children[0].querySelector("span") ||
			div.children[0].firstElementChild;
