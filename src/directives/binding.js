...
import { _sanitizeHtml, findContext } from "../dom.js";
import { _execStatement, evaluate } from "../evaluate.js";
import { _config, _onDispose, _warn, _watchExpr } from "../globals.js";
import { _disposeChildren, registerDirective } from "../registry.js";
...
registerDirective("bind-*", {
	...
	init(el, name, expr) {
		...
		function update() {
			...
			if (val != null)
				el.setAttribute(attrName, String(_sanitizeAttrValue(attrName, val)));
			else el.removeAttribute(attrName);
		}
		_watchExpr(expr, ctx, update);
		update();
