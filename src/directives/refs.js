...
registerDirective("use", {
	...
	init(el, _name, tplId) {
		...
		for (const child of [...el.children]) {
			...
			if (!slots[slotName]) slots[slotName] = document.createDocumentFragment();
		...
		const clickHandler = async (e) => {
			...
			try {
				...
				if (successTpl) {
					...
					if (clone) {
						const tplEl = document.getElementById(successTpl.replace("#", ""));
				...
				if (redirectPath && _routerInstance) _routerInstance.push(redirectPath);
				...
				_warn(
					`call ${method.toUpperCase()} ${resolvedUrl} failed:`,
					err.message,
				);
