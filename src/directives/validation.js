...
import { _onDispose, _validators } from "../globals.js";
import {
	_disposeChildren,
	processTree,
	registerDirective,
				...
				switch (name) {
					...
					case "min":
						if (Number(value) < Number(args[0]))
							msg = `Minimum value is ${args[0]}`;
						break;
					case "max":
						if (Number(value) > Number(args[0]))
							msg = `Maximum value is ${args[0]}`;
						break;
					case "custom":
...
function _clearErrorTemplate(anchorEl) {
	...
	const _existing = document.querySelector(`div[style="display: contents;"]`);
...
registerDirective("validate", {
	...
	init(el, _name, rules) {
		...
		if (el.tagName === "FORM") {
			...
			function checkValidity() {
				...
				for (const field of getFields()) {
					...
					const fieldErrorClass =
						field.getAttribute("error-class") || errorClassAttr;
					if (fieldErrorClass) {
					...
					if (topError && fieldInteracted) {
						...
						const tplRef =
							(perRuleVal?.startsWith("#") ? perRuleVal : null) ||
							(genericVal?.startsWith("#") ? genericVal : null);
						if (tplRef) {
			...
			function _updateSubmitButtons(form, isValid) {
				...
				for (const btn of buttons) {
					...
					if (
						...
						if (val !== "disabled" && val !== "true" && val !== "false")
							continue;
					}
			...
			const hasFieldValidateOn = [...getFields()].some((f) =>
				f.hasAttribute("validate-on"),
			);
...
registerDirective("error-boundary", {
	priority: 1,
	init(el, _name, fallbackTpl) {
		const ctx = findContext(el);

		function showFallback(message) {
			const clone = _cloneTemplate(fallbackTpl);
			if (clone) {
				const childCtx = createContext({ err: { message } }, ctx);
				_disposeChildren(el);
				el.innerHTML = "";
				const wrapper = document.createElement("div");
				wrapper.style.display = "contents";
