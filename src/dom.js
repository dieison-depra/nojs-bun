...
export async function _loadRemoteTemplates(root) {
	...
	_log(
		"[LRT] called on",
		scope === document ? "document" : scope.nodeName || "fragment",
		"— found",
		templates.length,
		"template[src]",
		[...templates].map((t) => t.getAttribute("src")),
	);
	if (!templates.length) return;
	const promises = [...templates].map(async (tpl) => {
		if (tpl.__srcLoaded) {
			_log("[LRT] SKIP (already loaded):", tpl.getAttribute("src"));
			return;
		}
		...
		try {
			...
			if (!tpl.hasAttribute("route") && tpl.parentNode) {
				...
				for (const child of children) {
					if (child.nodeType === 1)
						child.__srcBase = child.__srcBase || baseFolder;
...
export async function _loadTemplateElement(tpl) {
	const src = tpl.getAttribute("src");
	if (tpl.__srcLoaded) {
		_log("[LTE] SKIP (already loaded):", src);
		return;
	}
	_log(
		"[LTE] START fetch:",
		src,
		"| route:",
		tpl.hasAttribute("route"),
		"| inDOM:",
		document.contains(tpl),
		"| loading:",
		tpl.getAttribute("loading"),
	);
	tpl.__srcLoaded = true;
	const resolvedUrl = _resolveTemplateSrc(src, tpl);
	_warnIfInsecureTemplateUrl(resolvedUrl, src);
	...
	try {
		...
		if (!tpl.hasAttribute("route")) {
			...
			const warmups = [...nested].map((sub) => {
				...
				if (_templateHtmlCache.has(subUrl)) return null;
				return fetch(subUrl)
					.then((r) => {
...
export async function _loadRemoteTemplatesPhase1(defaultRoutePath) {
	...
	const phase1 = all.filter((tpl) => {
		...
		return (
			defaultRoutePath != null && tpl.getAttribute("route") === defaultRoutePath
		);
	});
